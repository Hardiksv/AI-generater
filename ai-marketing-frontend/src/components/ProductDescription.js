...
const ProductDescription = ({ goBack }) => {
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try {
      const response = await fetch("https://ai-generater-1.onrender.com/api/description", {
        method: "POST",
      ...
      setDescription(data.description);  // ✅ uses backend key correctly
    } catch (error) {
      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-12 relative w-full max-w-6xl z-10">
        ...
        >
          ...
          <p className="text-gray-500 text-center mb-6">
            Perfect for eCommerce, marketing, or showcasing your product.
          </p>
